...
def open_specified_layers(model, open_layers):
    ...
    Args:
    - model (nn.Layer): neural net model.
...
def count_num_param(model):
    ...
    if hasattr(model, 'classifier') and isinstance(model.classifier, nn.Layer):
...
def one_hot(labels_train):
    ...
    labels_train_1hot_size = list(labels_train.shape) + [nKnovel, ]
    labels_train_unsqueeze = labels_train.unsqueeze(labels_train.dim())
    labels_train_1hot = paddle.zeros(labels_train_1hot_size, dtype=labels_train.dtype)
    labels_train_1hot = paddle.put_along_axis(
        labels_train_1hot,
        labels_train_unsqueeze,
        paddle.ones_like(labels_train_unsqueeze),
        len(labels_train_1hot_size) - 1
    )
    return labels_train_1hot.astype('float32')
    # return nn.functional.one_hot(labels_train, nKnovel)
